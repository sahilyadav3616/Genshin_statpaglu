...
function openDetail(index){
  ...
  modal.querySelector('.detail-talents').innerHTML = c.talents.length ? c.talents.map(t=>`<div class="detail-stat"><span>${escapeHTML(t.name)}</span><b>Lv. ${t.level}</b></div>`).join('') : '<p class="detail-empty">No talent data available.</p>';
...
function renderRoster(filterText){
  ...
  filtered.forEach(({c,i},pos)=>{
    ...
    el.querySelector('.talents').textContent=`TALENTS  ${talents.map(t=>t.level).join(' / ')||'—'}`;
