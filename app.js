...
(function initBannerScene(){const scene=document.querySelector('#bannerScene');if(!scene)return;const vesna=scene.querySelector('[data-role="vesna"]'),v=scene.querySelector('[data-role="vodyanitsa"]');if(!vesna||!v)return;const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)');const p={x:0,y:0,a:false};let raf=0,t0=performance.now(),paused=false;const clamp=(n,a,b)=>Math.min(b,Math.max(a,n));function tick(now){if(!paused){const t=(now-t0)/1000,px=p.a?p.x:0,py=p.a?p.y:0,bob=Math.sin(t*.75)*7,drift=Math.sin(t*.42)*18,vx=clamp(drift+px*16,-34,34),vy=clamp(bob+py*10,-18,18);vesna.style.transform=`translate3d(${vx}px,${vy}px,0)`;v.style.transform=`translate3d(${clamp(vx*.62+Math.sin(t*1.15)*6,-28,28)}px,${clamp(vy*.72+18+Math.sin(t*.9)*4,4,40)}px,0)`;scene.style.setProperty('--scene-px',px);scene.style.setProperty('--scene-py',py)}raf=requestAnimationFrame(tick)}if(!reduced?.matches){scene.addEventListener('pointermove',e=>{const r=scene.getBoundingClientRect();p.x=clamp((e.clientX-(r.left+r.width/2))/(r.width/2),-1,1);p.y=clamp((e.clientY-(r.top+r.height/2))/(r.height/2),-1,1);p.a=true},{passive:true});scene.addEventListener('pointerleave',()=>{p.x=0;p.y=0;p.a=false},{passive:true});document.addEventListener('visibilitychange',()=>paused=document.hidden,{passive:true});raf=requestAnimationFrame(tick)}else{vesna.style.transform='translate3d(0,0,0)';v.style.transform='translate3d(0,18px,0)'}window.addEventListener('beforeunload',()=>cancelAnimationFrame(raf),{once:true})})();