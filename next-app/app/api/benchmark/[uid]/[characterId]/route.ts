...
export async function GET(_request: Request, { params }: { params: Promise<{ uid: string; characterId: string }> }) {
  ...
  try {
    const categoriesData = await getJson(`https://akasha.cv/api/v2/leaderboards/categories?characterId=${encodeURIComponent(characterId)}`);
    ...
    if (!calculationId) return NextResponse.json({ error:'No public world leaderboard is available for this character yet.' }, { status:404 });
    ...
    return NextResponse.json({
      ...
      weapon:top.weapon ? {
        ...
        refinement:Number(top.weapon.refinement ?? top.weapon.refine ?? 1),
